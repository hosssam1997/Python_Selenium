...
EMAIL_ID = "fcishossamsabry@gmail.com"
...
class BrowserstackCrawler(object):
   ...
   def signup(self):
      cookie_cta = self.browser.find_element(by=By.ID, value='accept-cookie-notification')
      cookie_cta.click()

      # Navigate to Signup Page
      button = self.browser.find_element(by=By.ID, value='signupModalButton')
      button.click()

      time.sleep(2)

      # Fill user's full name
      username = self.browser.find_element(by=By.ID, value='user_full_name')
      # username.send_keys('John Doe')
      self.slow_typing(username, 'Hossam Sabry')

      time.sleep(1)
      # Fill user's email ID
      email = self.browser.find_element(by=By.ID, value='user_email_login')
      self.slow_typing(email, EMAIL_ID)

      time.sleep(2)
      # Fill user's password
      password = self.browser.find_element(by=By.ID, value='user_password')
      ...
      toc = self.browser.find_element(by=By.NAME, value='terms_and_conditions')
      toc.click()

      # click on signup page
      signupbutton = self.browser.find_element(by=By.ID, value='user_submit')
      signupbutton.click()
   ...
   def validate_signupform(self):
       ...
       message_body_html_elements = self.browser.find_elements(by=By.NAME, value='msg-body')
